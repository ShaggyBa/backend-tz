...
describe('StickerController (unit)', () => {
  ...
  it('delete => if found should destroy and emit', async () => {
    ...
    expect((mockSocketManager.emitToSessionStickerDeleted as jest.Mock).mock.calls[0][0]).toBe(
      'session-2'
    );

    const deletedPayload = (mockSocketManager.emitToSessionStickerDeleted as jest.Mock).mock
      .calls[0][1];
