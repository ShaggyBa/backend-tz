  ...
  init(server: http.Server) {
    ...
    this.io.use(async (socket: TSocket, next) => {
      try {
        ...
        if (authTokenRaw) {
          token = authTokenRaw.startsWith('Bearer ') ? authTokenRaw.split(' ')[1] : authTokenRaw;
        } else {
  ...
  private async onConnection(socket: TSocket) {
    ...
    socket.on(
      ...
      async (
        ...
        cb?: (err?: string | null, ok?: { ok: true } | null) => void
      ) => {
        try {
          const rawTok = payload?.token ?? '';
          const rawToken =
            typeof rawTok === 'string' ? rawTok.replace(/^Bearer\s+/i, '') : undefined;
          if (!rawToken) {
