	...
	return async (req, res, next) => {
		try {
			...
			if (!sessionIdRaw) {
				...
				(req as any).sessionAccess = { allowed: false, owner: false, session: null, reason: 'Missing sessionId' };
				return next();
			}
			...
			if (!session) {
				(req as any).sessionAccess = { allowed: false, owner: false, session: null, reason: 'Session not found' };
			...
			if (requireOwner) {
				(req as any).sessionAccess = { allowed: false, owner: false, session, reason: 'Only owner allowed' };
